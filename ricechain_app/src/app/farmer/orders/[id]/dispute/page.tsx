...
export default function FarmerDisputePage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // 注文データを取得
                const orderResponse = await fetch(`/api/orders/sales/${params.id}`);
                if (!orderResponse.ok) {
                ...
                // この注文が現在の農家の商品を含むかチェック
                if (orderData.order.sellerId !== currentFarmerId) {
                    router.push('/farmer/orders');
                    return;
                }
                ...
                if (!disputeResponse.ok) {
                    ...
                    router.push(`/farmer/orders/${params.id}`);
                    return;
                }
            ...
            if (response.ok) {
                ...
                if (result.dispute) {
                    ...
                    const newMsg: ChatMessage = {
                        ...
                        senderName: order?.seller?.handleName || '販売者',
                        message: newMessage,
    ...
    const getCustomerName = () => {
        if (!order) return '顧客';
        return order.buyer?.handleName || order.shippingAddress?.name || '顧客';
    };
    ...
    // 現在の農家が紛争の販売者かどうか判断
    const isSeller = currentFarmerId === dispute.sellerId;
    const yourStatement = isSeller ? dispute.sellerStatement : dispute.buyerStatement;
    const theirStatement = isSeller ? dispute.buyerStatement : dispute.sellerStatement;
            ...
            <Card className="mb-6">
                <CardHeader>
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                        <div>
                            ...
                            <CardDescription>顧客名: {customerName}</CardDescription>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {/* Their Statement */}
                        <div>
                            <h3 className="text-sm font-medium text-gray-500 mb-2">{customerName}の申し立て</h3>
                        ...
                        <div className="mt-6 p-4 rounded-lg bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800/30">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center space-x-2">
                                    <div className="text-xs text-purple-700 dark:text-purple-300">買い手</div>
                                    ...
                                    <div className="text-xs text-purple-700 dark:text-purple-300">売り手</div>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                <div className="mt-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        <Button
                            ...
                            onClick={() => setNewMessage('ご不便をおかけして申し訳ありません。問題の詳細を確認いたします。')}
                        >
                            謝罪と確認
                        </Button>
                        <Button
                            variant="outline"
                            className="justify-start text-sm"
                            onClick={() => setNewMessage('代替品の発送または全額返金のどちらをご希望でしょうか？')}
                        >
                            解決策の提案
                        </Button>
                        <Button
                            variant="outline"
                            className="justify-start text-sm"
                            onClick={() => setNewMessage('発送時の状態について詳細をご説明いたします。')}
                        >
                            発送状態の説明
                        </Button>
                        <Button
                            variant="outline"
                            className="justify-start text-sm"
                            onClick={() => setNewMessage('お申し出の内容について理解いたしました。以下のように対応させていただきます：')}
                        >
                            解決に向けた提案
                        </Button>
                    </div>
